...
namespace TheraBytes.BetterUi.Editor {
    ...
    public class BetterRawImageEditor : GraphicEditor {
        ...
        protected override void OnEnable() {
            ...
            BetterRawImage img = target as BetterRawImage;
            materialDrawer = new ImageAppearanceProviderEditorHelper(serializedObject, img);

            m_UVRectContent = new GUIContent("UV Rect");
            m_Texture = serializedObject.FindProperty("m_Texture");

            textureSetingsFallbackProp = serializedObject.FindProperty("fallbackTextureSettings");
            textureSetingsCollectionProp = serializedObject.FindProperty("customTextureSettings");
        ...
        public override string GetInfoString() {
            BetterRawImage rawImage = target as BetterRawImage;
        ...
        /// <summary>
        ///     <para>
        ///         Implement specific RawImage inspector GUI code here. If you want to simply extend the existing editor call
        ///         the base OnInspectorGUI () before doing any custom GUI code.
        ///     </para>
        /// </summary>
        public override void OnInspectorGUI() {
            serializedObject.Update();

            ScreenConfigConnectionHelper.DrawGui("Texture Settings",
                textureSetingsCollectionProp,
                ref textureSetingsFallbackProp,
                DrawTextureSettings);
        ...
        public override void OnPreviewGUI(Rect rect, GUIStyle background) {
            if (Event.current.type == EventType.Repaint) {
                BetterRawImage betterRawImage = target as BetterRawImage;
                ...
                Texture2D rawTexture2D = new(rawTexture.width, rawTexture.height);
                ...
                Texture2D preview = new(previewWidth, previewHeight);
                if (drawCoordinates.width > 0 && drawCoordinates.height > 0) {
                    var pixels = rawTexture2D.GetPixels(textureCoordinates.x,
                        textureCoordinates.y,
                        textureCoordinates.width,
                        textureCoordinates.height);
                    preview.SetPixels(drawCoordinates.x,
                        drawCoordinates.y,
