...
namespace TheraBytes.BetterUi.Editor {
    ...
    public class RectTransformDataDrawer : PropertyDrawer {
        private static readonly int floatFieldHash = "FloatFieldHash".GetHashCode();
        ...
        public static float HeightWithAnchorsExpanded => 11.5f * EditorGUIUtility.singleLineHeight + 2;
        public static float HeightWithoutAnchorsExpanded => 9.5f * EditorGUIUtility.singleLineHeight + 2;

        public static void OverwritePushPullMethods(Action<RectTransform, RectTransformData> push,
                                                    Action<RectTransform, RectTransformData> pull) {
            pushCallback = push;
            pullCallback = pull;
        }
        ...
        private static void
            SmartPositionAndSizeFields(SerializedProperty prop, RectTransformData data, Rect rect, ref float yPos) {
            ...
            if (equalAnchorX) {
                EditorGUI.BeginProperty(columnRect,
                    null,
                    prop.FindPropertyRelative("AnchoredPosition").FindPropertyRelative("x"));
                FloatFieldLabelAbove(prop,
                    columnRect,
                    () => data.AnchoredPosition.x,
                    val => data.AnchoredPosition = new Vector2(val, data.AnchoredPosition.y),
                ...
                FloatFieldLabelAbove(prop,
                    columnRect,
                    () => data.OffsetMin.x,
                    val => data.OffsetMin = new Vector2(val, data.OffsetMin.y),
            ...
            if (equalAnchorY) {
                EditorGUI.BeginProperty(columnRect,
                    null,
                    prop.FindPropertyRelative("AnchoredPosition").FindPropertyRelative("y"));
                FloatFieldLabelAbove(prop,
                    columnRect,
                    () => data.AnchoredPosition.y,
                    val => data.AnchoredPosition = new Vector2(data.AnchoredPosition.x, val),
                ...
                FloatFieldLabelAbove(prop,
                    columnRect,
                    () => -data.OffsetMax.y,
                    val => data.OffsetMax = new Vector2(data.OffsetMax.x, -val),
                    DrivenTransformProperties.None,
                    new GUIContent("Top"));
            ...
            FloatFieldLabelAbove(prop,
                ...
                val => data.LocalPosition = new Vector3(data.LocalPosition.x, data.LocalPosition.y, val),
                DrivenTransformProperties.AnchoredPositionZ,
                Styles.transformPositionZContent);
            EditorGUI.EndProperty();
            ...
            if (equalAnchorX) {
                ...
                FloatFieldLabelAbove(prop,
                    columnRect,
                    () => data.SizeDelta.x,
                    val => data.SizeDelta = new Vector2(val, data.SizeDelta.y),
                    DrivenTransformProperties.SizeDeltaX,
                    equalAnchorX ? new GUIContent("Width") : new GUIContent("W Delta"));
                ...
                FloatFieldLabelAbove(prop,
                    columnRect,
                    () => -data.OffsetMax.x,
                    val => data.OffsetMax = new Vector2(-val, data.OffsetMax.y),
                    DrivenTransformProperties.None,
            ...
            if (equalAnchorY) {
                ...
                FloatFieldLabelAbove(prop,
                    columnRect,
                    () => data.SizeDelta.y,
                    val => data.SizeDelta = new Vector2(data.SizeDelta.x, val),
                    DrivenTransformProperties.SizeDeltaY,
                    equalAnchorY ? new GUIContent("Height") : new GUIContent("H Delta"));
                ...
                FloatFieldLabelAbove(prop,
                    columnRect,
                    () => data.OffsetMin.y,
                    val => data.OffsetMin = new Vector2(data.OffsetMin.x, val),
                    DrivenTransformProperties.None,
        ...
        private static void SmartAnchorFields(SerializedProperty prop, RectTransformData data, Rect rect, ref float yPos,
                                              ref bool anchorExpand) {
            Rect controlRect = GetControlRect(EditorGUIUtility.singleLineHeight * (!anchorExpand ? 1 : 3), rect, ref yPos);
            controlRect.x += 10;
            ...
            Vector2Field(controlRect,
                () => data.AnchorMin.x,
                val => data.AnchorMin.x = val,
                () => data.AnchorMin.y,
                val => data.AnchorMin.y = val,
                DrivenTransformProperties.AnchorMinX,
                DrivenTransformProperties.AnchorMinY,
                prop.FindPropertyRelative("AnchorMin").FindPropertyRelative("x"),
                prop.FindPropertyRelative("AnchorMin").FindPropertyRelative("y"),
                Styles.anchorMinContent);

            controlRect.y = controlRect.y + EditorGUIUtility.singleLineHeight;
            Vector2Field(controlRect,
                () => data.AnchorMax.x,
                val => data.AnchorMax.x = val,
                () => data.AnchorMax.y,
                val => data.AnchorMax.y = val,
                DrivenTransformProperties.AnchorMaxX,
                DrivenTransformProperties.AnchorMaxY,
                prop.FindPropertyRelative("AnchorMax").FindPropertyRelative("x"),
                prop.FindPropertyRelative("AnchorMax").FindPropertyRelative("y"),
        ...
        private static void SmartPivotField(SerializedProperty pivotProp, RectTransformData data, Rect rect, ref float yPos) {
            ...
            Vector2Field(controlRect,
                () => data.Pivot.x,
                val => data.Pivot.x = val,
                () => data.Pivot.y,
                val => data.Pivot.y = val,
                DrivenTransformProperties.PivotX,
                DrivenTransformProperties.PivotY,
                pivotProp.FindPropertyRelative("x"),
                pivotProp.FindPropertyRelative("y"),
                Styles.pivotContent);
        }
        ...
        private static Rect GetColumnRect(Rect totalRect, int column) {
            ...
            Rect rect = totalRect;
            rect.xMin = rect.xMin + ((totalRect.width - 4f) * (column / 3f) + column * 2);
        ...
        private static void Vector2Field(Rect position, Func<float> xGetter, Action<float> xSetter, Func<float> yGetter,
                                         Action<float> ySetter,
                                         DrivenTransformProperties xDriven, DrivenTransformProperties yDriven,
                                         SerializedProperty xProperty, SerializedProperty yProperty, GUIContent label) {
        ...
        private static void RotationField(SerializedProperty rotationProp, SerializedProperty eulerProp,
                                          SerializedProperty saveAsEulerProp, RectTransformData data, Rect rect, ref float yPos) {
            Rect controlRect = GetControlRect(EditorGUIUtility.singleLineHeight, rect, ref yPos);
            controlRect.x += 10;
            controlRect.width -= 10;
            ...
            Rect prefixRect =
                new(controlRect.x,
                    ...
                    controlRect.height); //EditorGUI.PrefixLabel(controlRect, new GUIContent(""));
            ...
            if (data.SaveRotationAsEuler) {
                ...
                FloatField(eulerProp,
                    rectX,
                    () => data.EulerAngles.x,
                    val => data.EulerAngles = new Vector3(val, euler.y, euler.z),
                    DrivenTransformProperties.Rotation,
                    Styles.X);
                FloatField(eulerProp,
                    rectY,
                    () => data.EulerAngles.y,
                    val => data.EulerAngles = new Vector3(euler.x, val, euler.z),
                    DrivenTransformProperties.Rotation,
                    Styles.Y);
                FloatField(eulerProp,
                    rectZ,
                    () => data.EulerAngles.z,
                    val => data.EulerAngles = new Vector3(euler.x, euler.y, val),
                    DrivenTransformProperties.Rotation,
                    Styles.Z);
                ...
                FloatField(rotationProp,
                    rectX,
                    () => data.Rotation.eulerAngles.x,
                    val => data.Rotation = Quaternion.Euler(val, euler.y, euler.z),
                    DrivenTransformProperties.Rotation,
                    Styles.X);
                FloatField(rotationProp,
                    rectY,
                    () => data.Rotation.eulerAngles.y,
                    val => data.Rotation = Quaternion.Euler(euler.x, val, euler.z),
                    DrivenTransformProperties.Rotation,
                    Styles.Y);
                FloatField(rotationProp,
                    rectZ,
                    () => data.Rotation.eulerAngles.z,
                    val => data.Rotation = Quaternion.Euler(euler.x, euler.y, val),
                    DrivenTransformProperties.Rotation,
                    Styles.Z);
        ...
        private static void FloatFieldLabelAbove(SerializedProperty prop, Rect position, Func<float> getter, Action<float> setter,
                                                 DrivenTransformProperties driven, GUIContent label) {
            FloatField(prop, position, getter, setter, driven, label, true);
        }

        private static void FloatField(SerializedProperty prop, Rect position, Func<float> getter, Action<float> setter,
                                       DrivenTransformProperties driven, GUIContent label, bool labelAbove = false) {
            float val = getter();
        ...
        private class Styles {
            ...
            public static readonly GUIStyle measuringLabelStyle = new("PreOverlayLabel");
            public static readonly GUIContent anchorsContent = new("Anchors");

            public static readonly GUIContent anchorMinContent = new("Min",
                "The normalized position in the parent rectangle that the lower left corner is anchored to.");

            public static readonly GUIContent anchorMaxContent = new("Max",
                "The normalized position in the parent rectangle that the upper right corner is anchored to.");

            public static readonly GUIContent positionContent = new("Position",
                "The local position of the rectangle. The position specifies this rectangle's pivot relative to the anchor reference point.");

            public static readonly GUIContent sizeContent = new("Size", "The size of the rectangle.");

            public static readonly GUIContent pivotContent = new("Pivot",
                "The pivot point specified in normalized values between 0 and 1. The pivot point is the origin of this rectangle. Rotation and scaling is around this point.");

            public static readonly GUIContent transformScaleContent = new("Scale",
                "The local scaling of this Game Object relative to the parent. This scales everything including image borders and text.");

            public static readonly GUIContent transformPositionZContent = new("Pos Z",
                "Distance to offset the rectangle along the Z axis of the parent. The effect is visible if the Canvas uses a perspective camera, or if a parent RectTransform is rotated along the X or Y axis.");
