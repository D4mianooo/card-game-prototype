...
namespace TheraBytes.BetterUi.Editor {
    public class ExampleScenesPage : WizardPage {
        ...
        public override string NameId => "ExampleScenesPage";

        protected override string NextButtonText
        {
            get
            {
                var packages = elements.OfType<InstallPackageSelectionWizardPageElement>();
        ...
        protected override void OnInitialize() {
            ...
            Add(new InfoWizardPageElement(
                "This is the oldest example scene of Better UI. It shows the most releant features and is devided into categories: " +
            ...
            if (IsFallbackLandscape()) {
                Add(new InfoWizardPageElement("This Example scene was created for Portrait resolutions as fallback. " +
                                              "Your fallback is Landscape, so the example may look broken.",
                    InfoType.ErrorBox));
            }
            ...
            Add(new InfoWizardPageElement(
                "This example shows some animation features as well as a movable mask where the masked sprite in the hierarchy below stays at a certain position. " +
            ...
            Add(new InfoWithButtonWizardPageElement(
                "This example shows realtime detection of screen sizes and portrait / landscape detection. " +
                "You may preview this example in your browser.",
                "Preview",
                () => Application.OpenURL("https://documentation.therabytes.de/better-ui-webgl-dpi-test/container.html")));
            if (IsFallbackPortrait()) {
                Add(new InfoWizardPageElement("This Example scene was created for Landscape resolutions as fallback. " +
                                              "Your fallback is Portrait, so the example may not work as intended.",
                    InfoType.ErrorBox));
            }
            else if (!ResolutionMonitor.Instance.OptimizedScreens.Any(o => o.Name == "Portrait")) {
                Add(new InfoWizardPageElement(
                    "For this example to work properly, it is required to have a screen configuration named 'Portrait'.",
                    InfoType.WarningBox));
            }
            ...
            Add(new InfoWizardPageElement(
                "You will find just a long text and a toggle in this exampke. If you hit the toggle everything becomes big. " +
                "This is achieved through custom screen tags in screen configurations."));
            Add(new InfoWizardPageElement(
                "If you install this example, another screen configuration 'Accessibility Mode' is added to the Resolution Monitor. " +
                "Note that it will not be removed again if you remove this example. You may want to remove it manually later.",
                InfoType.WarningBox));
            ...
            wizard.DoReloadOperation(this,
                () =>
                {
                    ...
                    if (customScreenTagExample.SelectionState == InstallSelectionState.Install
                        ...
                        ScreenTypeConditions screenCondition = new("Accessibility Mode");
        ...
        private InstallPackageSelectionWizardPageElement CreateSelection(string exampleName) {
            string package = Path.Combine(Application.dataPath,
                string.Format("TheraBytes/BetterUI/packages/Example_{0}.unitypackage", exampleName));
            string folder = Path.Combine(Application.dataPath, string.Format("TheraBytes/BetterUI/Example/{0}", exampleName));

            InstallPackageSelectionWizardPageElement result = new(exampleName, package, folder);
            result.MarkComplete();
