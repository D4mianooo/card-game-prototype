...
namespace TheraBytes.BetterUi.Editor {
    public class ThirdPartySupportPage : WizardPage {
        private const string PACKAGE_PATH_TMP_OLD =
            "TheraBytes/BetterUI/packages/BetterUI_TextMeshPro_UiEditorPanel.unitypackage";

        private const string PACKAGE_PATH_TMP_NEW =
            "TheraBytes/BetterUI/packages/BetterUI_TextMeshPro_EditorPanelUI.unitypackage";
        ...
        public override string NameId => "ThirdPartySupportPage";
                ...
                if (foundTmpVersion) {
                    ...
                    if (v1 > 3 || v1 == 3 && v2 > 0 || v1 == 3 && v2 == 0 && v3 > 6) {
                        Add(new InfoWizardPageElement("The latest tested TextMesh Pro version is 3.0.6.\n" +
                                                      "It is very likely that the Better UI add-on for TextMesh Pro will also work with version " +
                                                      versionString +
                    ...
                    if (v1 < 1 || v1 == 1 && v2 == 0 && v3 < 54) {
                        Add(new InfoWizardPageElement(
                            "Your version of TextMesh Pro is too old and not supported. Please upgrade at least to version 1.0.54.",
                            InfoType.ErrorBox));
                    }
                    else {
                        const string assertionText =
                            "You have a TextMesh Pro package installed which is not supported in your version of Unity. You should upgrade TextMesh Pro.";
                        bool isNewBaseClass =
                        ...
                        (o, v) =>
                        {
                            if (GUILayout.Button("Import Add-On (TextMesh Pro " + textNewVersion + ")")) {
                                wizard.DoReloadOperation(this,
                                    () =>
                                    {
                                        AssetDatabase.ImportPackage(Path.Combine(Application.dataPath, PACKAGE_PATH_TMP_NEW),
                                            false);
                            ...
                            if (GUILayout.Button("Import Add-On (TextMesh Pro " + textOldVersion + ")")) {
                                wizard.DoReloadOperation(this,
                                    () =>
                                    {
                                        AssetDatabase.ImportPackage(Path.Combine(Application.dataPath, PACKAGE_PATH_TMP_OLD),
                                            false);
